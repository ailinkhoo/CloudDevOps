...
df_poly = pd.read_csv(file[0]) # add column 'type' to differentiate between uni and poly
df_poly.insert(6, "type", "Poly")
df_uni = pd.read_csv(file[1])
df_uni.insert(6, "type", "Uni")

df = pd.concat([df_poly, df_uni], ignore_index=True) # combine two dataframe into one (https://www.tutorialspoint.com/how-to-merge-multiple-csv-files-into-a-single-pandas-dataframe)

df.graduates = df.graduates.str.replace(',', '') # Remove commas in graduates column
df = df[(df.course == 'Information Technology') & (df.sex=="MF")] # Selecting all the rows in which course is Information Technology and sex is MF
df.graduates = pd.to_numeric(df.graduates) # Convert string to integer

print(df)
df_poly = df[df['type'] == 'Poly']
df_uni = df[df['type'] == 'Uni']

polygrad = df_poly['graduates'].sum() 
unigrad = df_uni['graduates'].sum() 

print("Poly IT graduates: ", polygrad, "\n", "Uni IT graduates: ", unigrad)

print("Total IT graduates from 2005 to 2020: ", polygrad + unigrad) 